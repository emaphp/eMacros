...
class ArrayShift implements Applicable {
	...
	public function apply(Scope $scope, GenericList $arguments) {
		if (count($arguments) == 0) throw new \BadFunctionCallException("ArrayShift: No arguments found.");
		$target = $arguments[0];
		...
		$ref = $target->symbol;
		if (is_array($scope->symbols[$ref])) return array_shift($scope->symbols[$ref]);
