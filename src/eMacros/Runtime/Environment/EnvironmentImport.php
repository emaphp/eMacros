...
class EnvironmentImport implements Applicable {
	public function apply(Scope $scope, GenericList $arguments) {
		...
		if ($nargs == 0) {
			throw new \BadFunctionCallException('EnvironmentImport: No package specified.');
		}
		...
		if ($rc->isSubclassOf('eMacros\Package\Package')) {
			if ($nargs > 1) {
				...
				//validate alias
				if (!preg_match(Symbol::PACKAGE_PREFIX, $alias . '::')) {
					throw new \InvalidArgumentException("EnvironmentImport: Alias '$alias' is not a valid package alias.");
				}
