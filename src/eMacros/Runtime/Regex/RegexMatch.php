...
class RegexMatch implements Applicable {
	...
	public function apply(Scope $scope, GenericList $arguments) {
		...
		if ($nargs < 2) {
			throw new \BadFunctionCallException("RegexMatch: Function expects at least 2 parameters.");
		}
		
		if ($nargs == 2) {
			return preg_match($arguments[0]->evaluate($scope), $arguments[1]->evaluate($scope));
		}
		else {
			$target = $arguments[2];
			...
			if ($nargs == 3) {
				return preg_match($arguments[0]->evaluate($scope), $arguments[1]->evaluate($scope), $scope->symbols[$ref]);
			}
			elseif ($nargs == 4) {
				return preg_match($arguments[0]->evaluate($scope), $arguments[1]->evaluate($scope), $scope->symbols[$ref], $arguments[3]->evaluate($scope));
			}
			else {
